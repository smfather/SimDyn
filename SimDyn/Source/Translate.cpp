...
TranslateDlg* Translate::getDialog(const int format, const bool import)
{
	...
	QString formatFilter;// = QObject::tr(QString("%1").arg(format).toLatin1().constData());
	switch (format)
	{
	case 0: formatFilter = "BREP Files (*.brep *.rle)";
	case 1: formatFilter = "IGES Files (*igs *.iges)";
	case 2: formatFilter = "STEP Files (*.tep *.step)";
	case 3: formatFilter = "CSFDB Files (*.csfdb)";
	case 4: formatFilter = "VRML Files (*.vrml)";
	case 5: formatFilter = "STL Files (*.stl)";
	}
	...
	QString datadir = (QString(qgetenv("C:/SimDyn").constData()) + QString("/SimDyn/Data"));
	...
	if (!import)
	{
		...
		case FORAMT_STEP:
			...
			break;
		case FORMAT_CSFDB:
			myDlg->addMode(MgtBRep_WithTriangle, QObject::tr("Export With Triangles"));
			myDlg->addMode(MgtBRep_WithoutTriangle, QObject::tr("Export Without Triangles"));
			break;
		}
	}
...
bool Translate::exportCSFDB(const QString& file, const Handle(TopTools_HSequenceOfShape)& shapes)
{
	...
	{
		myInfo = QObject::tr("Can&apost save file %1.").arg(file);
		return false;
	}
	...
	if (data->ErrorStatus() != Storage_VSOk)
	{
		myInfo = QObject::tr("Can&apos;t store persistent data.");
		return false;
	}
	return true;
}
