...
void View::initCursors()
{
	...
	if (!zoomCursor)
		zoomCursor = new QCursor(QPixmap(":/Resources/cursor_zoom.png"));
	if (!rotCursor)
		rotCursor = new QCursor(QPixmap(":/Resources/cursor_rotate.png"));
}
...
void View::onLButtonUp(Qt::MouseButtons nFlags, const QPoint point)
{
	...
	activateCursor(myCurrentMode);
	SimDyn::getApplication()->onSelectionChanged();
}
...
{
	SimDyn* stApp = SimDyn::getApplication();
	...
	if (myContext->NbSelected())
	{
		...
		myToolMenu->addAction(aList->at(SimDyn::ToolWireframeId));
		myToolMenu->addAction(aList->at(SimDyn::ToolShadingId));
		myToolMenu->addAction(aList->at(SimDyn::ToolColorId));

		QMenu* myMaterMenu = new QMenu(myToolMenu);

		QList<QAction*>* aMeterActions = SimDyn::getApplication()->getMaterialActions();

		//QString dir = SimDyn::getResourceDir() + QString("/");
		myMaterMenu = myToolMenu->addMenu(QIcon(":/Resources/close.png"), tr("&amp;Shading"));
		for (int i = 0; i < aMeterActions->size(); i++)
			myMaterMenu->addAction(aMeterActions->at(i));

		myToolMenu->addAction(aList->at(SimDyn::ToolTransparencyId));
		myToolMenu->addAction(aList->at(SimDyn::ToolDeleteId));
